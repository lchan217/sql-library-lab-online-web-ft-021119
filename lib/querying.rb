...
def select_name_and_series_subgenres_of_authors
  ...
  JOIN authors ON series.subgenre_id = authors.id"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series
  JOIN characters ON series.id = characters.series_id WHERE characters.species = 'human' LIMIT 1"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, books.title FROM characters
  ...
  WHERE character_books.character_id = character_books.book_id"
end
